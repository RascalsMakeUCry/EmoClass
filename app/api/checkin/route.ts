...
export async function POST(request: NextRequest) {
  try {
    ...
    const validEmotions: EmotionType[] = ["happy", "normal", "stressed"];
    if (!validEmotions.includes(emotion as EmotionType)) {
      return NextResponse.json(
        {
          success: false,
          message:
            "Nilai emosi tidak valid. Silakan pilih salah satu emoji yang tersedia.",
        },
        { status: 400 }
      );
    }
    ...
    if (note && typeof note === "string" && note.length > 100) {
      return NextResponse.json(
        {
          success: false,
          message: "Catatan tidak boleh lebih dari 100 karakter",
        },
        { status: 400 }
      );
    }
    ...
    if (existingCheckin && existingCheckin.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Kamu sudah check-in hari ini. Silakan coba lagi besok.",
        },
        { status: 400 }
      );
    }
